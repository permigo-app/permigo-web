...
// `tour` = ancre du didacticiel (OnboardingTour surligne l'élément visible)
const NAV_ITEMS: { href: string; labelKey: string; fullLabelKey?: string; tour?: string; icon: React.ReactNode }[] = [
  ...
  { href: '/panneaux', labelKey: 'nav_panneaux', tour: 'nav-panneaux', icon: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  { href: '/examen', labelKey: 'nav_examen', fullLabelKey: 'examen_blanc', tour: 'nav-examen', icon: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  { href: '/profil', labelKey: 'nav_profil', tour: 'nav-profil', icon: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function SidebarItem({ href, icon, label, active, tour }: {
  href: string; icon: React.ReactNode; label: string; active: boolean; tour?: string;
}) {
  return (
      ...
      <nav className="hidden lg:flex flex-col"
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, marginBottom: 4 }}>
          {NAV_ITEMS.slice(0, 4).filter(item => item.href !== '/lecons').map(item => (
            <SidebarItem key={item.href} href={item.href} icon={item.icon} label={t(item.fullLabelKey ?? item.labelKey)} active={isNavActive(item.href, pathname)} tour={item.tour} />
          ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <SidebarItem href={NAV_ITEMS[4].href} icon={NAV_ITEMS[4].icon} label={t(NAV_ITEMS[4].labelKey)} active={isNavActive(NAV_ITEMS[4].href, pathname)} tour={NAV_ITEMS[4].tour} />
          {premium ? (
